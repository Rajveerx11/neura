...
| Automatic execution | Implemented in source; unreleased | Work and YOLO proof share the network-disabled sandbox, exact offline pins, and no host fallback. Automatic Git disables hooks, filters, text conversion, external diff, prompts, and fs monitors; checkpoints copy raw bytes outside Git. Default MCP configuration has no local command. `/ship` only verifies; it does not publish Git changes. Delivery evidence remains [#22](https://github.com/Rajveerx11/neura/issues/22). |
