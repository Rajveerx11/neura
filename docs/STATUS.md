...
| Installation | Blocked; incremental managed-file safety in source | Release manifest hashes, staged Learn dependencies, managed-file recovery journal, launcher/preflight drift checks, and explicit user extension hashes are covered by synthetic tests. Activation across Pi, user settings, launcher, and Terminal is not atomic; live rehearsal and full [#21](https://github.com/Rajveerx11/neura/issues/21) remain open. |
