...
- Hardened public-repository CI with explicit Windows 2025 and Ubuntu 24.04
  ...
  package and produces a deterministic source archive plus SPDX SBOM snapshots for both
  lockfiles, and checksums. Documented current GitHub controls without treating
  them as production evidence and retained private package manifests. Replaced
  the unlicensed launch image with the owner-supplied Neura logo, recorded its
  provenance and hash, and added fresh synthetic-data Plan and Learn screenshots
  generated by the real browser verification suites.
