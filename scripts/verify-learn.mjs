...
try {
  ...
  check(installer.indexOf('if (-not (Test-Command "npm"))') < firstMutation && installer.indexOf("[version]'24.10.0'") < firstMutation, "Installer validates prerequisites before copying live files");
  check(installer.includes("Learn document runtime dependencies missing") && installer.includes("Learn SQL lab"), "Drift check covers Learn runtime and Node compatibility");
  await app.fire("session_start");
