...
await fs.writeFile(path.join(vault, "Welcome.md"), "untouched\n");
const previousNeura = process.env.NEURA;
const previousLearnVault = process.env.NEURA_LEARN_VAULT;
...
try {
  ...
  check((await fs.readdir(path.join(vault, "Neura", "_System", "events"))).length === 1, "Lesson did not automatically create one vault concept event");
  check(await fs.readFile(path.join(vault, "Welcome.md"), "utf-8") === "untouched\n", "Automatic capture changed unrelated vault content");
  ...
  await app.fire("input", { source: "interactive", text: "Please show examples first." });
  await denied(() => app.call("learn_progress", { action: "preference", preferenceKey: "exampleTiming", preferenceValue: "first", evidence: "different text" }), /complete latest message/);
  await app.call("learn_progress", { action: "preference", preferenceKey: "exampleTiming", preferenceValue: "first", evidence: "Please show examples first." });
  let personalizedPrompt = await app.fire("before_agent_start", { systemPrompt: "base" });
  check(!personalizedPrompt.systemPrompt.includes('"exampleTiming":"first"'), "One model-extracted message activated a preference");
  await app.fire("input", { source: "interactive", text: "Start with a concrete example for me." });
  await app.call("learn_progress", { action: "preference", preferenceKey: "exampleTiming", preferenceValue: "first", evidence: "Start with a concrete example for me." });
  personalizedPrompt = await app.fire("before_agent_start", { systemPrompt: "base" });
  check(personalizedPrompt.systemPrompt.includes('"exampleTiming":"first"'), "Two distinct exact learner messages did not activate the preference");
  const vaultEventTexts = await Promise.all((await fs.readdir(path.join(vault, "Neura", "_System", "events"))).map((name) => fs.readFile(path.join(vault, "Neura", "_System", "events", name), "utf-8")));
  check(!vaultEventTexts.some((text) => text.includes('"answer"')), "Automatic vault capture exported raw learner answers");
  ...
  if (previousLearnVault === undefined) delete process.env.NEURA_LEARN_VAULT; else process.env.NEURA_LEARN_VAULT = previousLearnVault;
