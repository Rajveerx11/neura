...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;overflow-x:hidden;background:var(--page);color:var(--text);font:16px/1.62 var(--sans);text-rendering:optimizeLegibility}a{color:var(--plan);text-underline-offset:3px}a:focus-visible{outline:3px solid var(--focus);outline-offset:3px;border-radius:2px}code{color:var(--focus);font-family:var(--mono)}
    .topbar{position:sticky;top:0;z-index:5;display:flex;align-items:center;justify-content:space-between;gap:20px;min-height:52px;padding:0 22px;border-bottom:1px solid var(--line);background:rgba(8,10,15,.95);backdrop-filter:blur(8px)}.brand,.date,.eyebrow,section>header>span,.visual-index,.step-number{font-family:var(--mono);text-transform:uppercase;letter-spacing:.11em}.brand{color:var(--copper);font-size:12px;font-weight:700}.date{color:var(--dim);font-size:12px}nav{display:flex;min-width:0;max-width:100%;gap:8px;overflow-x:auto}nav a{display:inline-flex;align-items:center;min-height:44px;padding:0 6px;color:var(--muted);font-size:12px;text-decoration:none;white-space:nowrap}
