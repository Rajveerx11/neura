...
function ownership(manifest) {
  ...
  if (exists(asTarget(stateName))) {
    ...
    if (previous.schemaVersion !== 1 || !previous.files || Array.isArray(previous.files)) throw Error('invalid previous install state');
...
function recover() {
  ...
  if (!Array.isArray(entries)) throw Error('invalid install journal; manual recovery required');
  for (const [index, name] of entries.entries()) {
    const target = asTarget(name);
    const backup = path.join(transaction, 'backup', String(index));
...
function prepare() {
  if (exists(path.join(transaction, 'journal.json'))) recover();
  else if (exists(transaction)) fs.rmSync(transaction, { recursive: true, force: true });
...
function activate() {
  ...
  const stale = old ? Object.keys(old.files).filter(name => !Object.hasOwn(receipt.files, name)) : [];
